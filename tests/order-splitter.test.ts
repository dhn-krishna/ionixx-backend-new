...
test("rejects invalid order type", () => {
  assert.throws(
    () => splitOrder({ type: "HOLD" as never, amount: "100", portfolio }),
...
test("rejects bad amount", () => {
  assert.throws(
    () => splitOrder({ type: "BUY", amount: "10.999", portfolio }),
    /at most 2 decimal places/,
  );
});

test("rejects non-string market prices as bad requests", () => {
  assert.throws(
    () =>
      splitOrder({
        type: "BUY",
        amount: "100",
        portfolio: [
          { symbol: "AAPL", weight: "100", marketPrice: 100 as never },
        ],
      }),
    /Invalid market/,
  );
